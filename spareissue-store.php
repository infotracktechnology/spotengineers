...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $amount = $con->query("SELECT grand_total FROM job_entry WHERE id = '$job_id'")->fetch_object()->grand_total;
    $grand_total =  $amount+array_sum($_POST['total']);

    $issue_id = $con->insert_id;
    $job = $con->query("UPDATE `job_entry` SET `status` = 'spare issue', `grand_total` = '$grand_total' WHERE `id` = '$job_id'");
