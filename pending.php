...
$sql = "SELECT j.job_no, j.job_date, c.name AS customer_name, c.phone AS customer_phone, e.name AS employee_name FROM job_entry j LEFT JOIN customer c ON j.customer_id = c.id LEFT JOIN employee e ON j.emp_id = e.id WHERE status='pending' ORDER BY j.id ASC;";
